...
func newRouter(authSvc *auth.Service, blockSvc *block.Service, jotSvc *jot.Service, habitSvc *habit.Service, broker *pubsub.Broker, cfg routerConfig) *http.ServeMux {
	...
	mux.Handle("GET /habits/week", web.RequireSession(authSvc, frontend.HabitWeekHandler(habitSvc)))
	...
	mux.Handle("POST /habits/reorder", web.RequireSession(authSvc, frontend.HabitReorderHandler(habitSvc)))
