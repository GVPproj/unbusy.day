# Per-user Boards: owner-scoped cards and a user-keyed broker

Each User privately owns their Cards. `card` gains an `owner_id`, the uniqueness
constraint becomes `UNIQUE(owner_id, position)` (so two Users can both hold
position 0), every `cards.Service` query is scoped by owner, and the in-process
`pubsub.Broker` is **keyed by user** — `cards.Event` and the `Publisher`
interface carry an owner key so a reorder fans out only to that User's
subscribers. We rejected a single global board behind a login gate, and rejected
...
- Migration adds `owner_id`, swaps the unique constraint, and **deletes the old
  ...
  inserts cards.
- Cards now need **generated unique ids** (hand-picked ids can't repeat across
  Users).
- New Users are **seeded** with starter cards on first login, so the reorder demo
  works before any create-card UI exists. An add/delete-card UI is deferred.
