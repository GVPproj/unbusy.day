...
- **Tool versions have one source each**: templ in `go.mod` (CLI installed via `go list -m`), Datastar and Motion as exact CDN tags in their call sites, and CodeMirror's direct inputs in `frontend/vendorcodemirror/main.go` (which generates the resolved manifest). `task check:versions` flags drift for all of them. For CodeMirror upgrades, read `docs/agents/codemirror.md`.
