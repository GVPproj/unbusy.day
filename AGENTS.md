...
AGENTS.md-aware tool). `CLAUDE.md` is a symlink to this file so both entry points
read identical content.

**Agent config layout:** this file is the single source of top-level guidance.
Skills live once under `.agents/skills/` (`linear/`, `verify/`); `.claude/skills`
is a symlink to it, so Claude Code and pi both see the same skill set. Shared
consumer docs are `docs/agents/` and the `CONTEXT.md` glossary. Claude-Code-only
settings stay in `.claude/settings.local.json`.
...
- **`frontend/`** — Datastar + templ route handlers. `PageHandler` server-renders both panels (the block column and the Jotpad's stored text); `EventsHandler` is the SSE read path (first frame is the full column, so any (re)connect is made whole); the `POST /blocks*` endpoints each respond with an SSE element-patch of the committed column. `POST /jot` is the exception: it answers a JSON `{version, text?}` CAS ack that the client's sync driver applies itself (`text` present only when the store differs — merge or rejection); nothing may re-render an editor being typed into, so live jot state rides `/events` as `_jotv`/`_jott` **signal** patches, never element patches. The Jotpad editor is CodeMirror 6 (`js/jot/cm.js`, pinned esm.sh imports) over the shared save/convergence driver `js/jot/sync.js` (debounce, CAS, retry/backoff, save-state indicator, teardown beacon). Views split into `layouts/` (document shell), `routes/` (pages), `components/` (fragments/patch targets). `static/` is `go:embed`-served at `/static/`, organised by asset type: `css/app.css`, `fonts/`, `icons/` (favicon, PWA icons, og image), and `js/` — feature-grouped as `js/blocks/` (gestures.js entry + pointer/keyboard/rename/grid/commit, push.js, keyboard-reducer.js), `js/jot/` (cm.js, sync.js), `js/guide/` (demo.js, swipe.js), plus the standalone `js/now.js`, `js/shortcuts.js`, `js/invoker-fallback.js`. `manifest.webmanifest` and `sw.js` stay at the `static/` root (the service worker is re-served from `/sw.js` for PWA scope). URL paths mirror the folders — `/static/css/app.css`, `/static/js/blocks/gestures.js`. `RequireSession` middleware gates the app routes.
- **`auth/`** — passwordless email-OTP over DB-backed sessions (ADRs 0001/0002). `Mailer` is the email seam; dev uses `LogMailer` (codes on stdout). Signup is open — a correct code mints the account on first verify; the blast-radius bound is layered on the send path: per-email throttle, suppression list (ADR 0009), MX check, Turnstile presence check, per-IP/global rate limit, and a global send ceiling. `guardOpenSignup` in `main.go` refuses to boot a live mailer without Turnstile + ceiling unless `OPEN_SIGNUP_INSECURE=1`. Sessions are opaque tokens in an HttpOnly SameSite=Lax cookie. Nothing is seeded — a new user lands on an empty day grid whose free slots each carry an add button.
...
- Drag/stretch lives behind the `initBlockGestures(list, announce)` seam in `frontend/static/js/blocks/gestures.js`, with its implementation in sibling modules (`pointer.js` = Motion drag + stretch, `keyboard.js` = grab/resize/delete/rename, `rename.js` = shared inline editor, `grid.js` = DOM↔layout helpers, `commit.js` = the one gesture-end path both adapters call: write layout, refresh ARIA mirrors, announce, steer focus, dispatch). Motion is pinned via a CDN import in `js/blocks/pointer.js`; it transforms the real `<li>`, previews the client-computed push cascade (`push.js`, ADR 0005), settles as a FLIP, then commits through `commit.js`, which dispatches one `layout` event carrying the full layout. Listeners are delegated to `#block-list` so wiring survives morphs; block state is persisted in `data-span`/`data-slot` attributes so server render and client gesture agree.
...
- **Prefer the Go standard library.** Don't add a dependency until demonstrably necessary; the current deps (templ, modernc.org/sqlite, goose, datastar-go, sergi/go-diff) are the deliberate minimum.
...
- **Tool versions have one source each**: templ in `go.mod` (CLI installed via `go list -m`), Datastar client bundle, Motion, and the CodeMirror packages as exact CDN tags in the templ shells / `js/blocks/pointer.js` / `js/jot/cm.js`. `task check:versions` flags drift for all of them.
