...
Guidance for coding agents working in this repository (Claude Code, pi, and any
AGENTS.md-aware tool).

Skills live once under `.agents/skills/` (`.claude/skills` symlinks them);
shared consumer docs in `docs/agents/`, domain glossary in `CONTEXT.md`.

## What this is

unbusy.day — a Cal Newport-style time-blocking day planner (Fly app name `hello-cards`, module `github.com/GVPproj/unbusy.day`). The production stack and build are deliberately **Go-only**: business logic lives exactly once on the server, a colocated SQLite file is the source of truth (ADR 0007), SSE carries live reads, and the frontend is server-rendered **Datastar + templ**, not an SPA. Small client modules own browser interaction, including the deliberate client-computed Push exception in ADR 0005. Node is test tooling only. There is no CSS build step: styling is one hand-authored `app.css` (ADR 0011); templ generation is the only codegen. `CONTEXT.md` is the domain glossary; `docs/adr/` holds the decisions.
...
- `task test` — `go test ./...` plus the JS unit tests under `internal/frontend/static/js/`. Single Go test: `go test ./internal/block -run TestSetLayout`. CI runs `go test -race`; `task test:browser:smoke` runs the critical Chromium gate and `task test:browser` runs the full suite.
...
- **`block/`** — the transport-agnostic core `Service` over `*sql.DB`. All mutation logic lives here once; adapters never touch SQL. Mutations (`SetLayout`, `SetBounds`, `Create`, `Delete`, `Clear`, `Rename`) run in a write transaction and fan a full-state `block.Event` to the broker **post-commit**. `SetLayout` takes the whole client-computed layout and validates invariants via `ValidateLayout` (same block set, in bounds, no overlaps, span ≥ 1); `Create` has its own occupied-slot check. SQLite's `_txlock=immediate` serializes writers (ADRs 0005/0007). The `Publisher` interface is the pub/sub seam (nil = skip fan-out in unit tests).
- **`jot/`** — the Jotpad `Service` (`Get`/`Set`) over `jot`/`jot_version` columns on `"user"`, owner-scoped like everything else. Deliberately separate from `block`: a Jotpad has none of its invariants (no overlap, no push, no span), so folding it in would only make that module wider and shallower. Concurrent writers reconcile by compare-and-swap on the version: a current-version write replaces, a stale one is three-way merged (sergi/go-diff) against the base it was typed on (a one-revision `jot_base` shadow row). Same post-commit `Publisher` seam as `block` (nil = no fan-out in unit tests).
...
- **`frontend/`** — Datastar + templ route handlers. `PageHandler` server-renders both panels (the block column and the Jotpad's stored text); `EventsHandler` is the SSE read path (first frame is the full column, so any (re)connect is made whole); the `POST /blocks*` endpoints each respond with an SSE element-patch of the committed column. `POST /jot` is the exception: it answers a JSON `{version, text?}` CAS ack that the client's sync driver applies itself (`text` present only when the store differs — merge or rejection); nothing may re-render an editor being typed into, so live jot state rides `/events` as `_jotv`/`_jott` **signal** patches, never element patches. Habit commits ride `/events` as user-keyed invalidation signals; each open view re-reads its own selected week through authenticated `GET /habits/week`, whose week/refresh/view-keyed selector prevents stale responses from morphing `#habit-grid`. The Jotpad editor is CodeMirror 6 (`js/jot/cm.js`, self-hosted graph under `static/vendor/codemirror/`) over the shared save/convergence driver `js/jot/sync.js` (debounce, CAS, retry/backoff, save-state indicator, teardown beacon). Views split into `layouts/` (document shell), `routes/` (pages), `components/` (fragments/patch targets). `static/` is `go:embed`-served at `/static/`, feature-grouped into `js/blocks`, `js/jot`, `js/guide` plus a few standalone files — see the tree. URL paths mirror the folders (`/static/js/blocks/gestures.js`), and `sw.js` is re-served from `/sw.js` for PWA scope. `RequireSession` middleware gates the app routes.
- **`auth/`** — passwordless email-OTP over DB-backed sessions (ADRs 0001/0002). `Mailer` is the email seam; dev uses `LogMailer` (codes on stdout). Signup is open — a correct code mints the account on first verify; the blast-radius bound is layered on the send path: per-email throttle, suppression list (ADR 0009), MX check, Turnstile presence check, per-IP/global rate limit, and a global send ceiling. `guardOpenSignup` in `cmd/unbusy/auth.go` refuses to boot a live mailer without Turnstile + ceiling unless `OPEN_SIGNUP_INSECURE=1`. Sessions are opaque tokens in an HttpOnly SameSite=Lax cookie. No starter Blocks are seeded — a new User lands on an empty Day Plan whose free Slots each carry an add button.
...
- Theming is three axes on `:root` (tokens in `app.css`, attributes toggled in `internal/frontend/layouts/layout.templ`): **colorscheme** (`data-colorscheme`) selects the palette family, **colormode** (`data-colormode`) selects light/dark, and **feeling** (`data-feeling`) selects the font/icon set. All three persist to localStorage with a pre-paint script to avoid FOUC. New styles must use the tokens (`--bg`, `--ink`, `--surface`, `--accent`, …), never hardcoded colors.
- **Plain CSS in one hand-authored stylesheet** (ADR 0011): `internal/frontend/static/css/app.css`, committed and `go:embed`-served as the single cached `<link>` — SSE patches never re-ship CSS, and CSS never renders inside a patched fragment. Cascade layers own ordering (`reset, tokens, base, layout, components`); the `components` layer is a small shared class tier (`.btn`, `.field`, `.option-row`, …) plus one `@scope` block per leaf component (`.login-main`, `.app-dialog`, `.sidenav`, `.blocks`), bare element selectors inside, section comments naming the templ file each styles. Markup carries semantic hooks only — the JS hook classes (`.block-item`, `.open`, `.dragging`, …) double as styling anchors; conditionals use `templ.KV`; no templ `css` blocks. Grow the shared tier deliberately, not by default. Media queries: `@media (width < 52rem)` is the mobile breakpoint, `@media (pointer: coarse)` the touch variants.

## Code style

- **Keep comments succinct — 1 or 2 lines** — and only where they state something the code can't (a constraint, a browser quirk, a why). Prefer self-explanatory code and well-named variables/functions over comments.
- **Prefer the Go standard library.** Don't add a dependency until demonstrably necessary; the current direct deps (templ, modernc.org/sqlite, goose, datastar-go, sergi/go-diff, golang.org/x/time) are deliberate.
