...
- **`jot/`** — the Jotpad `Service` (`Get`/`Set`) over a `jot` column on `"user"`, owner-scoped like everything else. Deliberately separate from `block`: a Jotpad has none of its invariants (no overlap, no push, no span), so folding it in would only make that module wider and shallower. No `Publisher` — cross-device live sync is deferred, so the page load is the only read.
...
- **`frontend/`** — Datastar + templ route handlers. `PageHandler` server-renders both panels (the block column and the Jotpad's stored text); `EventsHandler` is the SSE read path (first frame is the full column, so any (re)connect is made whole); the `POST /blocks*` endpoints each respond with an SSE element-patch of the committed column. `POST /jot` is the exception: 204, no patch — nothing re-renders a textarea the user may be typing into. Views split into `layouts/` (document shell), `routes/` (pages), `components/` (fragments/patch targets). `static/` is `go:embed`-served at `/static/` (app.css, block-gestures.js + `gestures/`, push.js, keyboard-reducer.js, now.js, jot.js, sw.js). `RequireSession` middleware gates the app routes.
...
- **Tool versions have one source each**: templ in `go.mod` (CLI installed via `go list -m`), Datastar client bundle and Motion as exact CDN tags in the templ shells / `gestures/pointer.js`. `task check:versions` flags drift for all of them.
