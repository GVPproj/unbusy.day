...
- Drag/stretch lives behind the `initBlockGestures(list, announce)` seam in `frontend/static/block-gestures.js`, split into a `gestures/` implementation folder (`pointer.js` = Motion drag + stretch, `keyboard.js` = grab/resize/delete/rename, `rename.js` = shared inline editor, `grid.js` = DOM↔layout helpers, `commit.js` = the one gesture-end path both adapters call: write layout, refresh ARIA mirrors, announce, steer focus, dispatch). Motion is pinned via a CDN import in `gestures/pointer.js`; it transforms the real `<li>`, previews the client-computed push cascade (`push.js`, ADR 0005), settles as a FLIP, then commits through `commit.js`, which dispatches one `layout` event carrying the full layout. Listeners are delegated to `#block-list` so wiring survives morphs; block state is persisted in `data-span`/`data-slot` attributes so server render and client gesture agree.
...
- **Tool versions have one source each**: templ in `go.mod` (CLI installed via `go list -m`), Datastar client bundle, Motion, and the CodeMirror packages as exact CDN tags in the templ shells / `gestures/pointer.js` / `jot-cm.js`. `task check:versions` flags drift for all of them.
