...
// Event is one mutation fanned out over the in-process pub/sub: the full
// ordered card list. Carries structured Cards rather than serialized bytes so
// each adapter renders it its own way off one published event.
type Event struct {
	Cards []Card `json:"cards"`
}
