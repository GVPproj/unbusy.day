...
// ErrNotSameCards signals a proposed layout whose ids are not exactly the
// owner's current card set (missing, unknown, or duplicate id).
var ErrNotSameCards = errors.New("layout is not the owner's current card set")

// ErrOutOfBounds signals a run [slot, slot+span) outside the day's bounds.
var ErrOutOfBounds = errors.New("card placed outside the day's bounds")

// ErrOverlap signals two cards' runs sharing at least one slot.
var ErrOverlap = errors.New("cards overlap")
...
// ErrBoundsOccupied signals a shrink that would strand a card outside the day.
var ErrBoundsOccupied = errors.New("bounds change strands a card outside the day")

// Placement is one card's proposed run of slots: [Slot, Slot+Span).
...
// (ADR 0005): same card set as current, every run within bounds, no overlaps.
// Pure — no DB or transport dependencies.
func ValidateLayout(bounds Bounds, current []Card, proposed []Placement) error {
	if len(proposed) != len(current) {
		return ErrNotSameCards
	}
