...
unbusy.day — a Cal Newport-style time-blocking day planner (Fly app name `hello-cards`, module `github.com/GVPproj/unbusy.day`). The stack is deliberately **Go-only**: business logic lives exactly once on the server, a colocated SQLite file is the source of truth (ADR 0007), SSE carries live reads, and the frontend is server-rendered **Datastar + templ** — no SPA, no client-side business logic, no Node. The one build step besides templ is CSS: a Tailwind v4 standalone binary compiles utilities. `CONTEXT.md` is the domain glossary; `docs/adr/` holds the decisions.

## Commands

Day-to-day uses [go-task](https://taskfile.dev) (`task`):

- `task dev` — full dev loop over a local SQLite file: templ watch with a reload proxy on **:7331** driving `go run ./cmd/unbusy` on `$PORT` (default :8080), plus `tailwindcss --watch`. Browse **:7331**, not the app port. Do **not** run `air` alongside it.
...
- `task build` — templ + css + `go build -o tmp/unbusy ./cmd/unbusy`.
...
- Theming is two axes of CSS custom properties on `:root` (`frontend/layouts/layout.templ`): **colorscheme** (`data-colorscheme`) sets color tokens, **feeling** (`data-feeling`) sets font/icon set. Both persist to localStorage with a pre-paint script to avoid FOUC. New styles must use the tokens (`--bg`, `--ink`, `--surface`, `--accent`, …), never hardcoded colors.
- **Utility-first CSS via Tailwind v4** (standalone binary; version pinned in `versions.env`). Component styling is markup-first — inline utilities, no single-use class names; state classes like `.open`/`.dragging` stay as JS hooks styled via arbitrary variants (`[&.open]:…`). Tokens are bridged via `@theme inline` in `frontend/input.css`; the scan covers `**/*.templ` only. Generated `output.css` is git-ignored and `go:embed`-served as one cached `<link>` — SSE patches never re-ship CSS. CSS survives **only** for foundational, non-component things (design tokens, `@font-face`, `@keyframes`/animation drivers, `::backdrop`/SVG filters), each a small co-located `<style>`/`@scope` island with a one-line comment saying why.
...
- **Tool versions have one source each**: templ in `go.mod` (CLI installed via `go list -m`), Tailwind in `versions.env`, Datastar client bundle and Motion as exact CDN tags in the templ shells / `drag.js`. `task check:versions` flags drift for all of them.
