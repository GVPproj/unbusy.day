...
// Seeder gives a fresh user their starter cards on first login (ADR 0003);
// *cards.Service satisfies it.
...
// starter cards, sets the session cookie, and redirects to the board. A bad
...
func VerifyCodeHandler(a AuthService, seeder Seeder, secureCookies bool) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := seeder.Seed(r.Context(), sess.UserID); err != nil {
			log.Printf("ds seed cards: %v", err)
			http.Error(w, "internal error", http.StatusInternalServerError)
			return
		}
