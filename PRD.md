...
- **M3b — Cloudflare proxy + Cache Rules** · ☑
  ...
  - **Shipped at `https://unbusy.day` (apex; `www` serves the same app).** Zone on CF Free; registrar (Hover) nameservers swapped; Hover parking A records (apex/`www`/wildcard) deleted, Hover MX kept. Verified: `/assets/*` MISS→HIT at both YVR and SIN colos (globalping), `index.html` `no-cache`→EXPIRED revalidation, `/api/*` DYNAMIC, and a mid-stream reorder's `id: <txid>` event delivered through CF on **both** `/api/events` and `/ds/events` (`curl --no-buffer -N`).
  - **Deviation — origin cert is Cloudflare Origin CA, not Let's Encrypt.** With the apex orange-clouded, CF Universal SSL publishes conflicting `_acme-challenge` TXT records that block Fly's LE DNS-01 validation (surfaced by `fly certs check`). Per Fly's Cloudflare doc: issued a 15-year Origin CA cert (apex + `*.unbusy.day`) via the CF API and `fly certs import`ed it — no renewal treadmill until 2041. Key/CSR/cert live outside the repo at `~/.config/unbusy.day/`. `_fly-ownership` TXT records added for apex and `www`.
  - **Token scope (§12):** zone-scoped with *DNS: Edit*, *Zone Settings: Edit*, *Cache Rules: Edit*, **plus *SSL and Certificates: Edit*** — the Origin CA `/certificates` endpoint needs the last one.
  - **"Disable buffering" note:** Free-plan Cache Rules expose no explicit response-buffering knob; bypass-cache alone proved sufficient (criterion verified empirically per §11). Rule expressions are written mutually exclusive so rule order can't change semantics.
