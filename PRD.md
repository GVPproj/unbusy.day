...
- **M3b — Cloudflare proxy + Cache Rules** · ☑
  ...
  - **Live at `https://unbusy.day` (apex) + `www`.** Zone on Cloudflare (NS `*.ns.cloudflare.com`), proxied apex (orange-cloud → CF anycast `104.21.*`/`172.67.*`), Fly custom certs for `unbusy.day` + `www.unbusy.day` **Verified** (SSL Full strict, Fly terminates TLS).
  - **Criterion 6 verified by curl (colo `YVR`):** `/assets/index-*.js` → `cf-cache-status: HIT` + `Cache-Control: public, max-age=31536000, immutable`; `index.html` → `no-cache` + revalidated; `/api/cards` and `/api/cards/reorder` → `DYNAMIC`; `/api/events` delivered `id: <txid>` + full card JSON ~1s after a POST, un-buffered; `/ds/events` delivered the authoritative templ fragment on connect and a fresh patch ~1s after a POST, un-buffered. One mutation fans to both adapters through CF.
  - **Deviation — zone configured via dashboard, not the §12 curl flow.** `cache-rules.json` is the canonical committed spec (authored to the CF cache-rules schema; `ops/cloudflare/README.md` has the apply + verify commands). It was **not** byte-captured from the live ruleset — that read needs `CLOUDFLARE_API_TOKEN`, which isn't on the deploy machine. Re-applying the JSON reproduces the verified behaviour.
  - **Remaining (cosmetic):** a true antipodal distant-colo spot-check (verified only from `YVR`; CF cache config is zone-global so behaviour is uniform — needs a foreign-region proxy/VPN to observe directly).

- **M3c — production hardening drills** · ☑ (criteria 5/7/8 + D5 + connection-ceiling record all done)
  ...
  - **Criterion 5 (restart) ✅** — through `https://unbusy.day`: order persisted across `fly machine restart` (seeded `a,b,c` → unchanged after; Neon is source of truth). `/healthz` back in **5s**. Reconnect with a stale `Last-Event-ID` correctly returned the **overflow sentinel** (`{"reason":"last-event-id outside replay window; refetch"}`) — the in-memory ring is wiped on restart, so the client falls back to full refetch per F2, not a bogus replay. Fresh reorder then delivered `id: <txid>` on the reconnected stream ~2s later.
  - **Criterion 7 (extended idle) ✅** — SSE held open through CF for **5m20s** with zero traffic; **13 `:keepalive` frames** at the 25s cadence (well past the ~100s intermediary idle window); the post-idle reorder delivered `id: <txid>` + full card list immediately, no refresh.
  - **Criterion 8 (schema drill) ✅** — done in `migrations/0002_card_created_at.sql`: additive `ADD COLUMN IF NOT EXISTS created_at`. Finding: neither FE observes the new column — API/SSE payloads are built from explicit column lists (`SELECT id, label, position`), not `SELECT *`, so a migration alone never changes the wire shape; exposing a field needs a code change + deploy. Expand-then-deploy ordering is safe by construction.
  - **D5 (rolling) ✅** — `fly.app.toml` carries `strategy = "rolling"` (switched from `immediate` now that SSE reconnect is verified).
  - **Connection-ceiling record ✅** — black-box SSE ramp (`ops/loadtest`, stdlib Go; forces HTTP/1.1 + no keep-alive so 1 virtual client = 1 origin TCP conn, hit `*.fly.dev` directly to measure the *app*, not CF pooling) against the live single `shared-cpu-1x` / **256 MB** machine in `sjc`. Stepped 250→500→1000→1500→2000→2500→**3000** concurrent held streams, firing a real `/api/cards/reorder` at each step and measuring fan-out completeness. **The FD/memory ceiling was never reached:** all **3000/3000** connections established, **0 failures**, machine never OOM'd or rebooted (uptime + `oom_killed=false` confirmed via `fly status`/event log), ~45s peak hold survived 13+ keepalive cycles. TTFB drifted 195 ms (150 conns) → **461 ms** (3000) — graceful, not a cliff.
    - **The binding constraint is fan-out *completeness* latency, not connection count.** "First event after reorder" stayed ~300–480 ms at every level, but the time to reach 90 % of held subscribers — the user-visible §2 "~1s cross-client sync" SLO — degrades with N: **699/675/671/829/772 ms at 250/500/1k/1.5k/2k**, then **1.52 s at 2500** and **~1.0 s at 3000**. Cause: one mutation = N serial socket write+flushes across N handler goroutines, all sharing **one** shared vCPU; that serialization, not FDs or RAM, is what stretches the tail (Publish itself is non-blocking — a saturated subscriber is dropped and recovers via `Last-Event-ID`, never stalls the origin).
    - **Data-driven §9 trigger:** on one `shared-cpu-1x`, fan-out holds comfortably inside the ~1s SLO to **~2000 concurrent SSE clients** and starts brushing/exceeding it by **~2500**. So the scale-out (or `fly scale vm` vertical bump) trigger is **sustained concurrent SSE clients approaching ~2000** — gated by fan-out latency (the SLO that breaks first), well before the FD/connection ceiling (untouched at 3000). Re-run anytime: `go run ./ops/loadtest -probe-each` (see `ops/loadtest/main.go`).
...
- **Scale-out** (multi-machine): in-process pub/sub no longer crosses instances. **Trigger (measured M3c):** not FD/connection exhaustion — one `shared-cpu-1x` holds 3000 idle SSE conns fine — but **fan-out latency**: per-mutation delivery to ~all subscribers stays inside the §2 ~1s SLO to **~2000 concurrent SSE clients** and slips past it by ~2500 (one shared vCPU serially writing N frames). Bump `fly scale vm` (vertical) or take a path below (horizontal) as sustained concurrency approaches ~2000. Two paths:
...
- **Connection-count bottleneck on Go app.** Each SSE client = one TCP conn; 10k clients = 10k FDs on `shared-cpu-1x`. Mitigation: vertical scale (`fly scale vm`) until §9 horizontal lands. **Measured (M3c):** the FD/memory ceiling is *not* the binding limit — one `shared-cpu-1x`/256 MB held **3000 concurrent SSE conns, 0 failures, no OOM**. What breaks first is **fan-out completeness latency** (one shared vCPU serially writing N frames per mutation): inside the ~1s sync SLO to **~2000 clients**, brushing/over 1s by ~2500. So the data-driven scale-out trigger is ~2000 sustained concurrent SSE clients, latency-gated, not FD-gated. Drill tool: `ops/loadtest`.
