...
// displaced blocks slide down by the minimum distance, gaps absorb first —
// or null when any block would be forced past the end of the day.
export function pushLayout(bounds, current, moved) {
	// Fixed runs nothing may overlap; the moved block's run is fixed first,
	// then each settled block's. Sweeping the others in ascending slot order
