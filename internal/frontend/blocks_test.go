// Datastar blocks-handler tests: a fake BlockService stands in for
// *block.Service; templ rendering is real, pinning observable wire behavior.
package frontend
...
func authedRequest(method, target string, body string) *http.Request {
	...
	return req.WithContext(web.WithOwner(req.Context(), testOwner))
}
...
// blocks/gestures.js reads slot/span from these attributes to seed each gesture.
